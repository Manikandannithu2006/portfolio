...
def main():
    ...
    if st.session_state.guess == st.session_state.low or st.session_state.guess == st.session_state.high:
        if feedback != 'Correct':
            st.write("Hmm... it seems like we are stuck, let's restart the game!")
            st.session_state.low = 1
            st.session_state.high = 100
            st.session_state.attempts = 0
